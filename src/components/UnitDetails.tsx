...
const UnitDetails: React.FC<UnitDetailsProps> = ({ unit, onClose }) => {
      ...
      <div className="grid grid-cols-4 gap-2 mt-4">
        {unit.ammo.map((ammo) => (
          <div key={ammo.type} className="flex flex-col items-center">
            <div className="h-24 w-6 bg-gray-800 rounded-sm relative flex flex-col-reverse">
              <div
                className={`w-full rounded-sm ${ammo.current < 30 ? 'bg-red-500' : 'bg-green-500'}`}
  ...
  return (
    <div className="bg-gray-900 text-white rounded shadow-lg overflow-hidden w-80 border border-gray-800">
