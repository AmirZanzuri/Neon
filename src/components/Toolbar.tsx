      ...
      <div className="bg-[#1a2638]/95 backdrop-blur-sm rounded-t-md px-2 py-1 flex items-center space-x-4">
        {buttons.map((button, index) => (
          <React.Fragment key={button.id}>
            <button
              className="p-1.5 rounded-sm hover:bg-[#2d4266]/95 text-gray-300 hover:text-white transition-colors"
            ...
            {index < buttons.length - 1 && index % 3 === 2 && (
              <div className="h-6 w-px bg-[#2d4266]/80"></div>
            )}
          </React.Fragment>
        ))}
        ...
        <button className="p-1.5 rounded-sm hover:bg-[#2d4266]/95 text-gray-300 hover:text-white transition-colors">
